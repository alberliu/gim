...
func InitClient(conn net.Conn) (tc *TcpClient) {
	tc.codec = NewCodec(conn)
	return
}
...
func (c *TcpClient) SignIn() {
	...
	pack := Package{Code: CodeSignIn, Content: signInBytes}
...
func (c *TcpClient) SyncTrigger() {
	...
	err = c.codec.Eecode(Package{Code: CodeSyncTrigger, Content: bytes}, 10*time.Second)
	if err != nil {
		fmt.Println(err)
	}
}
...
func (c *TcpClient) HeadBeat() {
	...
	for _ = range ticker.C {
		err := c.codec.Eecode(Package{Code: CodeHeadbeat, Content: []byte{}}, 10*time.Second)
		if err != nil {
			fmt.Println(err)
		}
	}
...
func (c *TcpClient) HandlePackage(pack Package) error {
	switch pack.Code {
	case CodeSignInACK:
		ack := pb.SignInACK{}
	...
	case CodeMessage:
		...
		err = c.codec.Eecode(Package{Code: CodeMessageACK, Content: ackBytes}, 10*time.Second)
		if err != nil {
			fmt.Println(err)
			return err
		}
...
func (c *TcpClient) SendMessage() {
	...
	err = c.codec.Eecode(Package{Code: CodeMessageSend, Content: bytes}, 10*time.Second)
	if err != nil {
		fmt.Println(err)
	}
}
